...
import type { PackedContext } from './contextPacking.service.js';
...
export const citationService = {
  ...
  formatPackedContextLine(packed: PackedContext): string {
    const timestampSpan = packed.chunkIds.length === 1
      ? formatTimestamp(packed.startTime)
      : `${formatTimestamp(packed.startTime)} - ${formatTimestamp(packed.endTime)}`;
    return `[${timestampSpan}] ${packed.text}`;
  },
};